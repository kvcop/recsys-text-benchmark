...
1. `feature_time_sec`: время подготовки признаков для всего корпуса `unique_news` в рамках прогона (offline-этап; для embeddings включает cold/warm работу с кэшем).
...
Для online-производительности ориентируйтесь на `eval_time_sec`, а для сравнения без offline-подготовки — на `train_time_sec + eval_time_sec`.
...
1. `MRR`: `0.3282 -> 0.3513` (`+7.02%`)
2. `NDCG@5`: `0.3080 -> 0.3283` (`+6.59%`)
3. `NDCG@10`: `0.3627 -> 0.3835` (`+5.73%`)
4. `Recall@5`: `0.4256 -> 0.4446` (`+4.45%`)
5. `Recall@10`: `0.5862 -> 0.6089` (`+3.88%`)

Времена того же прогона:

1. `feature_time_sec`: `1.032s -> 59.898s` (`+5706.44%`, `58.06x` хуже у embeddings)
2. `train_time_sec`: `30.652s -> 20.878s` (`-31.89%`, `1.47x` быстрее у embeddings)
3. `eval_time_sec`: `0.364s -> 0.029s` (`-91.95%`, `12.42x` быстрее у embeddings)
4. `total_time_sec`: `32.048s -> 80.805s` (`+152.14%`, `2.52x` хуже у embeddings)

Интерпретация времени:

1. `feature_time_sec` — это офлайн-подготовка фич по всему корпусу (`unique_news`), а не online latency одного запроса.
2. Если исключить `feature_time_sec`, то `Embeddings+SVM` быстрее: `train+eval` `31.016s -> 20.907s` (`-32.59%`, `1.48x` быстрее).

Резюме «стоит ли игра свеч»:

1. Да, если цель — максимум качества ранжирования и можно вынести построение embeddings в offline.
2. Скорее нет, если нужен быстрый cold-start/дешёвый запуск без отдельного GPU/кэша.
3. Практичный компромисс: `embeddings+SVM` как основной путь, `TF-IDF+SVM` как fallback.
